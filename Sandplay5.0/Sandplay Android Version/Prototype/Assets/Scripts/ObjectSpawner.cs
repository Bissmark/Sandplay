...
{
	public Camera 			UICamera; /* The NGUI camrea in the scene*/
	...
	void Update()
	{
		...
		case SPAWN_STATE.SCROLLING:
			...
			if (fScrollHoldTimer > 0.5f || IsTouchOnSandbox(t, out info) )
			{
				...
				//r = ViewCamera.ScreenPointToRay( UICamera.WorldToScreenPoint(startPos) );
		...
		case SPAWN_STATE.PREVIEW:
			...
			if ( t.fingerId == -1 || t.phase == TouchPhase.Ended )
			{
				...
				//r = ViewCamera.ScreenPointToRay( UICamera.WorldToScreenPoint(endPos) );
				//endPos =  r.GetPoint(10f);
			...
			if ( IsTouchOnSandbox( t, out info ) )
			{
				...
				if (oTween != null)
					oTween.Kill();
				oTween = HOTween.To(oCurrObject.transform, 0.3f, new TweenParms().Ease(EaseType.EaseOutCubic).Prop("forward", Vector3.forward));
	...
	{
		Ray r = ViewCamera.ScreenPointToRay (a_Touch.position);
