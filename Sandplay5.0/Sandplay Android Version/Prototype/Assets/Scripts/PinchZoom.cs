...
public class PinchZoom : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ( lookAtTarget )
        {
            Vector3 pos = new Vector3( target.transform.position.x, target.transform.position.y + lookAtOffset, target.transform.position.z );
            Quaternion targetRotation = Quaternion.LookRotation( pos - GetComponent<Camera>().transform.position );
